  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const upcomingResponse = await axios.get('/get-upcoming-appointments');
        setReportData(prevData => ({
          ...prevData,
          appointments: {
            ...prevData.appointments,
            upcoming: upcomingResponse.data.total // Set the upcoming appointments count
          }
        }));
      } catch (error) {
        console.error('Error fetching report data:', error);
  ...
  return (
    <div className="container mt-4">
      ...
      <div className="charts mt-4">
        ...
        {/* Bar Chart with Upcoming Appointments */}
        <BarChart width={600} height={300} className="bar-chart" data={[
          { name: 'Total Appointments', value: reportData.appointments.total },
          { name: 'Upcoming Appointments', value: reportData.appointments.upcoming }, // Upcoming Appointments
          { name: 'Total Feedback', value: reportData.feedbacks.total },
        ]}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="name" />
