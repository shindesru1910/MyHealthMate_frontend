...
const FileUpload = () => {
  ...
  const fetchUploadedFiles = async () => {
    try {
      ...
      setUploadedFiles(response.data.files || []);  // Ensure it's an array
    } catch (error) {
      console.error('Error fetching uploaded files', error);
  ...
  const handleUpload = async () => {
    ...
    try {
      const token = localStorage.getItem('token'); // Ensure 'token' matches the key used during login
      const response = await axios.post('upload-file', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
  ...
  return (
    <Container>
      <Title>Upload</Title>
      <Dropzone {...getRootProps()} role="button" aria-label="File upload dropzone">
      ...
      <FileList>
        {files.map((file) => (
          <FileItem key={file.name}>
            <span>{file.name}</span>
            <button onClick={() => removeFile(file.name)} aria-label={`Remove ${file.name}`}>
              Remove
            </button>
          </FileItem>
        ))}
      </FileList>
      <UploadButton onClick={handleUpload} disabled={uploading || files.length === 0} aria-label="Upload files">
        {uploading ? 'Uploading...' : 'Upload Files'}
      </UploadButton>
      {successFiles.length > 0 && (
        <SuccessMessage>
